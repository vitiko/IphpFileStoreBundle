...
class DefaultController extends Controller
{
    ...
    public function indexProtectedAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $photo = new PhotoProtected();
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($uploadForm->isValid()) {
                ...
                $em->persist($photo);
                $em->flush();
                return $this->redirect($this->generateUrl('photo_index_protected'));
            } else print "\n\n" . $uploadForm->getErrorsAsString();
        }

        return $this->render('TestBundle:Photo:index.html.twig', array(
            'uploadForm' => $uploadForm->createView(),
            'photos' => $em->getRepository('TestBundle:PhotoProtected')->findAll()
        ));
    }
